...
import type { ExpiryType, StrategyMarket } from '@ash/shared';
import { describeFixed, describeOpen, expiryLabel, fixedUnderlyings, isSpecific } from '@ash/shared';
...
/** Read-only chips for the fields a strategy fixes (shown in run forms instead of inputs). */
export function LockedFields({ market }: { market: StrategyMarket }) {
  const u = fixedUnderlyings(market);
  const chips: Array<{ label: string; value: string }> = [];
  if (u.length) chips.push({ label: u.length > 1 ? 'Basket' : 'Underlying', value: u.join(', ') });
  if (market.expiryType) chips.push({ label: 'Expiry', value: expiryLabel(market.expiryType) });
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {chips.map((c) => (
        <Tooltip key={c.label} content={c.label === 'Basket' ? { ...HELP.market.basket, note: HELP.market.locked.note } : HELP.market.locked}>
...
export function CustomStrikeSelect({
  ...
  const date = expiries.data?.find((e) => e.type === expiryType)?.date;
