...
import type { CalendarEntry, McxSettings } from '@/shared/mcx';
...
function ChannelsCard() {
  ...
  const settings = useQuery({ queryKey: ['mcx2-settings'], queryFn: mcxApi.settings });
  const channels = useQuery({ queryKey: ['mcx2-channels'], queryFn: mcxApi.channels });
  const [form, setForm] = useState<McxSettings | null>(null);
  ...
  const save = useMutation({
    mutationFn: (s: McxSettings) => mcxApi.saveSettings(s),
    onSuccess: (s) => {
      setForm(s);
      qc.invalidateQueries({ queryKey: ['mcx2-settings'] });
      qc.invalidateQueries({ queryKey: ['mcx2-channels'] });
      qc.invalidateQueries({ queryKey: ['mcx2-status'] });
    },
  });
  const test = useMutation({ mutationFn: (c: 'telegram' | 'email') => mcxApi.testChannel(c) });
    ...
    <Card className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <FieldLabel help={H.settings.cap} htmlFor="cap">
              Contracts / strategy
            </FieldLabel>
            <input id="cap" type="number" min={1} max={200} className="input w-full" value={form.universeCap} onChange={(e) => setForm({ ...form, universeCap: Number(e.target.value) })} />
          </div>
          <div>
...
function CalendarCard() {
  const qc = useQueryClient();
  const cal = useQuery({ queryKey: ['mcx2-calendar'], queryFn: mcxApi.calendar });
  const [entries, setEntries] = useState<CalendarEntry[] | null>(null);
  useEffect(() => {
    if (cal.data && !entries) setEntries(cal.data);
  }, [cal.data, entries]);
  const save = useMutation({
    mutationFn: (e: CalendarEntry[]) => mcxApi.saveCalendar(e),
    onSuccess: (e) => {
      setEntries(e);
      qc.invalidateQueries({ queryKey: ['mcx2-calendar'] });
      qc.invalidateQueries({ queryKey: ['mcx2-status'] });
    },
  });
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          <Tooltip content={H.settings.addHoliday}>
            <button type="button" className="btn-ghost py-1 text-xs" onClick={() => setEntries([...entries, { date: istToday(), kind: 'HOLIDAY', note: '' }])}>
              <CalendarOff className="w-3.5 h-3.5" /> Holiday
            </button>
          </Tooltip>
          <Tooltip content={H.settings.addSpecial}>
            <button type="button" className="btn-ghost py-1 text-xs" onClick={() => setEntries([...entries, { date: istToday(), kind: 'SPECIAL_SESSION', openMin: 18 * 60, closeMin: 19 * 60 + 15, note: '' }])}>
              <CalendarPlus className="w-3.5 h-3.5" /> Special session
            </button>
          </Tooltip>
        </div>
      </div>
      {entries.length === 0 && <p className="text-xs text-slate-500">No overrides — weekdays trade 09:00–23:30 (US summer) / 23:55 (US winter) IST.</p>}
      {entries.map((e, i) => (
