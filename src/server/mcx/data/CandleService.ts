 ...
 */
import type { McxInstrument, McxTimeframe, NativeInterval, SeriesSpec } from '@/shared/mcx';
...
import { buildSeries, type McxCandle, type RawCandle } from '../engine/candles';
...
/**
 * Calendar days fetched per native interval: ≥ ~300 candles of every timeframe
 * built from it (1h covers 300 × 4h; 1d covers ~140 weeks) with holiday margin.
 */
...
export class CandleService {
  ...
  private readonly built = new Map<string, McxCandle[]>();

  constructor(
    private readonly provider: McxDataProvider,
    private readonly calendar: McxMarketCalendar,
    readonly now: number,
  ...
  static nativeKey(instrument: McxInstrument, interval: NativeInterval): string {
    return `${instrument.token}|${interval}`;
  }

  /** Whether (instrument, interval) still needs a request this cycle. */
  needsFetch(instrument: McxInstrument, interval: NativeInterval): boolean {
  ...
  fetch(instrument: McxInstrument, interval: NativeInterval): Promise<RawCandle[]> {
  ...
  async fetchSeries(instrument: McxInstrument, tf: McxTimeframe): Promise<void> {
    await this.fetch(instrument, MCX2_TIMEFRAME[tf].native);
  }

  /** Synchronous lookup over what has been fetched (for the evaluator). */
  lookup: SeriesLookup = (instrument: McxInstrument, spec: SeriesSpec): SeriesResult => {
    const key = seriesKey(instrument, spec);
    const hit = this.built.get(key);
    if (hit) return { candles: hit };
    const native = this.done.get(CandleService.nativeKey(instrument, MCX2_TIMEFRAME[spec.timeframe].native));
    ...
    const candles = buildSeries(native, spec.timeframe, spec.candle, this.calendar, this.now);
