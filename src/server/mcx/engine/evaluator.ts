/**
 * Evaluates one unit (strategy version × target instrument) at a clock time T.
 *
...
export type SeriesLookup = (instrument: McxInstrument, spec: SeriesSpec) => SeriesResult;
...
export function compare(op: McxOperator, cur: [number, number], prev?: [number, number]): boolean {
  const [a, b] = cur;
  switch (op) {
    case 'GT':
      return a > b && !eq(a, b);
...
export function alignIndex(candles: McxCandle[], mode: EvaluationMode, at: number): number {
...
function seriesLabel(instrument: McxInstrument, s: SeriesSpec): string {
  const candle = s.candle.type === 'VOLUME' ? `Vol ${s.candle.volumePerCandle}` : (MCX2_CANDLE_TYPES.find((c) => c.type === s.candle.type)?.short ?? s.candle.type);
  return `${instrument.symbol} · ${MCX2_TIMEFRAME[s.timeframe].label} · ${candle}`;
}
...
function evalCondition(ctx: UnitContext, n: ConditionNode): ConditionTrace {
  ...
  const text = conditionText(n, input.definition.universe);
...
function evalPattern(ctx: UnitContext, n: PatternNode): ConditionTrace {
  const text = nodeText(n, ctx.input.definition.universe);
...
export function pendingReason(input: UnitEvalInput, now: number): string | undefined {
  ...
  const tf: McxTimeframe = input.definition.evaluation.triggerTimeframe;
  ...
  forEachLeaf(input.definition.expression, (leaf) => {
    ...
    for (const s of specs) {
      ...
      if (!last || last.time * 1000 < input.triggerOpenMs) {
        reason = `${instrument.symbol} ${MCX2_TIMEFRAME[tf].label} candle ${istTime(input.triggerOpenMs / 1000)} not available yet`;
        return;
      }
    }
  });
