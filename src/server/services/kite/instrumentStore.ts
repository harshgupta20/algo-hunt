...
export interface InstrumentSource {
  /** The full instrument master (F&O contracts of supported underlyings). */
  loadInstruments(): Promise<Instrument[]>;
  /** Live price used to derive the ATM strike (the front future's LTP). */
  referencePrice(future: Instrument): Promise<number>;
}

/** Warm serverless instances reuse the loaded master for this long. */
...
export class InstrumentStore {
  ...
  /** Drop the cache (after a sync) so the next load() re-reads the master. */
  invalidate(): void {
    this.loadedAt = 0;
  }
   ...
   * derived from the front future's live price and the underlying's strike
