 ...
 */
import type { AlertPolicy, SignalOutcome, TriState, UnitState } from '@/shared/v2';
import { MAX_ALERT_LAG_MS } from '@/shared/v2';
...
/** Signal identity: one signal per connection · strategy version · unit · trigger candle (· minute when repeats are allowed). */
export function signalIdentity(p: {
  ...
  const base = `${p.connectionId}:v${p.version}:${p.unitKey}:${p.triggerTimeframe}:${p.triggerCandle}`;
...
export function decide(i: PolicyInput): PolicyDecision {
  ...
  if (i.now - i.at > MAX_ALERT_LAG_MS) return { outcome: 'SUPPRESSED_STALE', next: signalled };
...
export function initialState(connectionId: string, unitKey: string): UnitState {
  return {
    connectionId,
    unitKey,
    state: 'IDLE',
