/**
 * Indicator + pattern computation over V2 series. The math is the shared,
 ...
 */
import type { Operand, PatternId, SourceField } from '@/shared/v2';
import { INDICATOR, PATTERN } from '@/shared/v2';
...
export function fieldValue(c: Candle, field: SourceField): number | undefined {
...
export function outputOf(op: IndicatorOperand): string | undefined {
  return op.output ?? INDICATOR[op.indicator]?.outputs?.[0]?.value;
}
...
export function patternValues(candles: Candle[], pattern: PatternId): Array<boolean | undefined> {
  const def = PATTERN_BY_ID[PATTERN[pattern].legacyId];
